    ...
    public static void main(String[] args) throws Exception {
        ...
        if (out.exists() && out.length() > 0) {
            ...
            String fix28_bridge =
                ...
                + "    P.$glVertexAttribPointer0 = (l,sz,t,norm,str,off) => gl.vertexAttribPointer(l,sz,t,!!norm,str,off);\n"
                ...
                + "    // Phase 3.13: singular buffer methods (libGDX GL20 convenience API)\n"
                ...
                + "        const P3 = WebGL30.prototype;\n"
                + "        P3.$b116 = (n,b) => { for(let i=0;i<n;i++) wib(b,i,alloc(gl.createVertexArray())); };\n"
                + "        P3.$a83  = v => gl.bindVertexArray(v ? get(v) : null);\n"
                + "        P3.$a304 = (n,b) => { for(let i=0;i<n;i++){const id=b.data?b.data[i]:0;gl.deleteVertexArray(get(id));free(id);} };\n"
                + "        P3.$glBindBuffer  = (t,b) => gl.bindBuffer(t, b ? get(b) : null);\n"
                + "        P3.$glBufferData  = (t,sz,data,u) => { if(data&&data.data) gl.bufferData(t,data.data,u); else if(sz>0) gl.bufferData(t,sz,u); };\n"
                + "        P3.$glDeleteBuffer = id => { gl.deleteBuffer(get(id)); free(id); };\n"
                + "    }\n"
